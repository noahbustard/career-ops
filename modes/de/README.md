...
Wenn die meisten deiner Stellenanzeigen auf Englisch sind, bleib bei den Standard-Modi unter `modes/`. Die englischen Modi greifen automatisch zu deutschen Anzeigen, sobald Codex sie als deutschsprachig erkennt — aber sie kennen die DACH-Marktbesonderheiten nicht im selben Detail.
...
Sag Codex zu Beginn der Session ausdrücklich:
...
Codex liest dann die Dateien aus diesem Ordner statt aus `modes/`.
...
Erinnere Codex in deiner ersten Session daran, dieses Feld zu respektieren ("Schau in `profile.yml`, ich habe `language.modes_dir` gesetzt"). Ab dann nimmt Codex automatisch die deutschen Modi.
